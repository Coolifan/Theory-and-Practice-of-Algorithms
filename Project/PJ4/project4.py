...
prim function will find the MST by assigning the proper vertex.prev values 
after execution.

INPUTS: 
adjList: the adjacency list of the Map class
adjMat: the adjacency matrix of the Map class
...
def prim(adjList, adjMat):
    ...
    startVertex = adjList[0]
    startVertex.cost = 0
    
    # Make the priority queue using cost for sorting
    # comparison operator < has been overloaded
    Q = MinQueue(adjList)
...
kruskal function will find the MST by getting the list of egdes that are in MST

INPUTS:
adjList: the given adjacency list of the Map class
edgeList: the given list of edge objects (already sorted)
...
def kruskal(adjList, edgeList):
    ...
    for vertex in adjList:
        makeset(vertex)
    # Initialize the empty MST. edgeList is already sorted.
    X = []
...
tsp function will trace the TSP tour using depth first search on the MST

INPUTS:
adjList: the adjacency list of the given Map class
start: the given start city vertex
...
def tsp(adjList, start):
    ...
    while len(stack) > 0:
        ...
        # Push the neighbors in MST onto the stack.
        # If they have not already been visited.
        for neighbor in current.mstN:
            if neighbor.visited == False:
                stack.append(neighbor)
                
    # Tour must end at the start vertex to complete the cycle.            
...
INPUTS:
v: the input vertex (root) that the singleton set will be created with
...
INPUTS:
v: the input vertex that will be used to find the set containing it

OUTPUTS:
v.pi: the parent (root) vertex
"""
def find(v):
    ##### Your implementation goes here. #####
    ...
    if not v.isEqual(v.pi):
        # Set our parent to be the root, which is also the root of our parent!
        v.pi = find(v.pi)
...
INPUTS:
u, v: the two vertices that will be used to make a union set
